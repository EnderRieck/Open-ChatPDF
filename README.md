...
### 🎨 AI 图像生成 (Tool Calling)
*   **智能调用**：AI 可以根据对话内容自动判断是否需要生成图片，并调用图像生成工具。
*   **实时状态**：调用工具时会显示"正在调用图像生成工具"的提示。
*   **灵活配置**：支持任何 OpenAI 兼容的图像生成 API（如 DALL-E、Stable Diffusion 等）。
*   **图片嵌入**：生成的图片会直接显示在 AI 回复的消息气泡中。
...
启用后，AI 可以在对话中自动调用图像生成工具来创建图片。

*   **启用开关**：在设置中打开"图像生成工具"开关。
*   **Base URL**：填写图像生成服务的地址（需支持 OpenAI 兼容的 `/chat/completions` 接口）。
*   **模型名称**：填写图像生成模型的名称（如 `dall-e-3`、`stable-diffusion` 等）。
*   **API Key**：可选，留空则使用主 API Key。

> 💡 **提示**：国内用户可使用支持图像生成的中转站服务。
